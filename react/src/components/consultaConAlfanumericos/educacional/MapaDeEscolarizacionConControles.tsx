// src/components/consultaConAlfanumericos/educacional/MapaDeEscolarizacionConControles.tsx
...
import { buscarPorComunidadesEnTerritorios, buscarPorTodasComunidadesEnTerritorios } from 'buscadores/paraAlfanumerica/dinamicas/Educacional';
...
const getColor = (value: number): string => {
  const red = 200;
  const green = Math.round(255 * (1 - (Math.log(value + 1))));
  const blue = 0;
...
const Legend = ({ min, max, escolaridad }: { min: number; max: number; escolaridad: string }) => {
  const map = useMap();
  ...
  useEffect(() => {
    ...
    legend.onAdd = () => {
      ...
      div.innerHTML = `
        <h4>Escolarización ${escolaridad}</h4>
        <div style="background: linear-gradient(to right, rgb(200, 255, 0), rgb(200, 0, 0)); width: 100px; height: 20px; border-radius: 5px;"></div>
        <div style="display: flex; justify-content: space-between;">
          <span>${min}%</span><span>${Math.round(max)}%</span>
        </div>
        <div>
          <i style="background: rgb(200, 255, 0); width: 12px; height: 12px; border-radius: 50%; display: inline-block; margin-right: 5px;"></i>Baja
          <br>
...
const MapaConControles: React.FC<MapaConControlesProps> = ({ datosEducacionales, datosParaConsulta, queEstoyViendo, modo }) => {
  ...
  const minPercentage = percentages ? Math.min(...Object.values(percentages)) : 0;
  const maxPercentage = percentages ? Math.max(...Object.values(percentages)) : 0;
  ...
  return (
    <>
      ...
      <MapContainer center={[0.969793, -70.830454]} zoom={zoomNivel} style={{ height: '600px', width: '100%', borderRadius: '3rem' }}>
        ...
        <Legend min={minPercentage} max={maxPercentage} escolaridad={opcionEscolaridad}/>
        <TileLayer
